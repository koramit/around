...
class SummaryCases extends AcuteHemodialysisAction
{
    public function __invoke(array $data, User|AvatarUser $user)
    {
        ...
        $report = $cases->transform(function ($case) use ($admissions) {
            ...
            return [
                ...
                'Total dialysis(ไม่นับ PE)' => $orders->count() - $orders->filter(fn ($order) => str_contains($order->meta['dialysis_type'], 'PE'))->count(),
                ...
                'Total PE' => $orders->filter(fn ($order) => str_contains($order->meta['dialysis_type'], 'PE'))->count(),
        ...
        return [
            'sheet' => $report,
            'filename' => "acute_HD-case_summary-{$dateStart->format('Y-m-d')}_to_{$dateEnd->format('Y-m-d')}.xlsx",
        ];
    }
}
